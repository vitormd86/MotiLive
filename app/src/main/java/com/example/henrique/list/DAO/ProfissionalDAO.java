...
public class ProfissionalDAO extends AsyncTask<Void, Void, Boolean> {
...
// prepared statement para inserção
        ...
        int id_profissao = profissional.getId_profissao();
        int id_pessoa = profissional.getId_pessoa();
        int id_especializacao = profissional.getId_especializacao();

        String registro = profissional.getRegistro();
        String url_linkedin = profissional.getUrl_linkedin();
        ...
        try {
            ...
            stmt = con.prepareStatement("INSERT INTO PROFISSIONAL(ID,ID_PROFISSAO,ID_PESSOA,ID_ESPECIALIZACAO,REGISTRO,URL_LINKEDIN) VALUES(?,?,?,?,?,?)");

            stmt.setInt(1, id);
            stmt.setInt(2, id_profissao);
            stmt.setInt(3, id_pessoa);
            stmt.setInt(4, id_especializacao);
            stmt.setString(5, registro);
            stmt.setString(6, url_linkedin);
    ...
    public void remove(Profissional profissional) throws SQLException, java.sql.SQLException {
// prepared statement para inserção
        int id = profissional.getId();
        try {

            PreparedStatement stmt;
            stmt = con.prepareStatement("DELETE FROM PROFISSIONAL WHERE ID="+id );
    ...
    public void update(Profissional profissional) throws SQLException, java.sql.SQLException {
// prepared statement para inserção
        int id = profissional.getId();
        int id_pessoa = profissional.getId_pessoa();
        try {
