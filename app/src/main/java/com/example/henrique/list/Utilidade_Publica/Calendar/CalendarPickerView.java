...
package com.example.henrique.list.Utilidade_Publica.Calendar;
...
import com.example.henrique.list.Utilidade_Publica.Calendar.MonthCellDescriptor.RangeState;
...
public class CalendarPickerView extends ListView {
  ...
  private int headerWeekTextColor;
  ...
  public CalendarPickerView(Context context, AttributeSet attrs) {
    ...
    titleTextColor = a.getColor(R.styleable.CalendarPickerView_titleTextColor,
        res.getColor(R.color.calendar_text_month));
    displayHeader = a.getBoolean(R.styleable.CalendarPickerView_displayHeader, true);
    headerWeekTextColor = a.getColor(R.styleable.CalendarPickerView_headerTextColor,
        res.getColor(R.color.calendar_text_weekDays));
    a.recycle();
  ...
  public FluentInitializer init(Date minDate, Date maxDate, Locale locale) {
    ...
    // maxDate is exclusive: bump back to the previous day so if maxDate is the first of a calendar_month,
    // we don't accidentally include that calendar_month in the view.
    maxCal.add(MINUTE, -1);
    ...
    while ((monthCounter.get(MONTH) <= maxMonth // Up to, including the calendar_month.
  ...
  /**
   * Set the typeface to be used for calendar_month titles.
   */
  ...
  @Override protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
    if (months.isEmpty()) {
      throw new IllegalStateException(
          "Must have at least one calendar_month to display.  Did you forget to call init()?");
    }
 ...
 /*     for (MonthCellDescriptor selectedCell : selectedCells) {
  ...
  /** Hold a cell with a calendar_month-index. */
  ...
  /** Return cell and calendar_month-index (for scrolling) for a given Date. */
        ...
        monthView =
            MonthView.create(parent, inflater, weekdayNameFormat, listener, today, dividerColor,
                dayBackgroundResId, dayTextColorResId, titleTextColor, displayHeader,
                    headerWeekTextColor, decorators, locale);
      } else {
