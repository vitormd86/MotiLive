...
public class PlanoDAO extends AsyncTask<Void, Void, Boolean> {
    ...
    public PlanoDAO(Context context) throws SQLException {
    ...
    public void adiciona(Plano plano) throws SQLException, java.sql.SQLException {
// prepared statement para inserção
        int id = plano.getId();
        int qtde_creditos = plano.getQtde_creditos();
        int qtde_dias = plano.getQtde_dias();

        double valor = plano.getValor();
        boolean status = plano.getStatus();
        boolean tipo = plano.getTipo();
        ...
        try {
            ...
            stmt = con.prepareStatement("INSERT INTO PLANO(ID,TIPO,QTDE_CREDITOS,QTDE_DIAS,VALOR,STATUS) VALUES(?,?,?,?,?,?)");

            stmt.setInt(1, id);
            stmt.setBoolean(2, tipo);
            stmt.setInt(3, qtde_creditos);
            stmt.setInt(4, qtde_dias);
